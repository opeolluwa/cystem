...
int main() {
  ...
  // Minimum weight in table (in pounds)
  // Maximum weight in table
  const unsigned min_ht{48};
  const unsigned max_ht{84};
  const unsigned ht_step{2};
  const size_t ht_count{1 + (max_ht - min_ht) / ht_step};
  // Minimum height in table (inches)
  // Maximum height in table
  ...
  for (auto h : height_ins) {
    // Stores BMI
    // Whole feet for output
    // Whole inches for output
